...
export class Resource extends BaseResource {
  ...
  public async create(params: Record<string, any>): Promise<ParamsType> {
    const instance = this.model.create(
      flat.unflatten(this.prepareParams(params))
    );
  ...
  public async update(
    ...
    if (instance) {
      ...
      for (const paramName of Object.keys(preparedParams)) {
        const prop = this.propsObject[paramName];
        // is ManyToMany
        if (prop && prop.reference() && prop.isArray()) {
          const idField = prop.subProperties().find((i) => i.isId());

          const repo = this.model
            .getRepository()
            .manager.getRepository(prop.reference());

          instance[paramName] = await repo.find({
            where: { [idField.name()]: In(preparedParams[paramName]) },
          });
        } else {
  ...
  private eagerRelations() {
    return this.properties()
      ...
      .map((prop) => prop.referenceName());
  }

  private prepareProps() {
    return parseEntityProperties(this.model.getRepository().metadata);
  }
